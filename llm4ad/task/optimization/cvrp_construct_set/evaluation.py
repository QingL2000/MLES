...
class CVRPSEvaluation(Evaluation):
    ...
    def evaluate_single(self, heuristic: callable, instance_information=None):
        ...
        return -(LLM_dis - baseline) / baseline


    def evaluate(self, heuristic: callable, ins_to_be_evaluated_id: Set | List | None=None, training_mode=True) -> Optional[dict, list]:
        ...
        for ins_id in ins_to_be_evaluated_id:
            ...
            each_evaluate_result = self.evaluate_single(heuristic, instance_information=instance_infomation)
            dis.append(each_evaluate_result)
            n_ins += 1
            instance_performance[ins_id] = {
                'score': each_evaluate_result
            }
